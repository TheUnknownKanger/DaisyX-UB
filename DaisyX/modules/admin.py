...
from Assist import BOT, MASTER
from DaisyX import BOTLOG, BOTLOG_CHATID, CMD_HELP
from DaisyX.utils import admin_cmd, errors_handler, register

# =================== CONSTANT ===================
PP_TOO_SMOL = "`The image is too small`"
PP_ERROR = "`Failure while processing the image`"
NO_ADMIN = (
    "I am not an admin!! @admins or @admin please make me admin i am very cool boy"
)
...
async def ban(bon):
    ...
    if reason:
        await bon.edit(
            f"BANNED 🔥`{str(user.id)}` was banned by {BOT} My Master {MASTER}  !!\n\nReason: {reason}"
        )
    else:
