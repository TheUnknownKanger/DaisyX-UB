...
from DaisyX import CMD_LIST, CMD_HELP
import io
 
from DaisyX.utils import remove_plugin,load_module
...
# kyu sir kang krne m musil aa rhi h kya ... Bolo help kr du kya 😂😂😂
                        ...
                        await event.edit("Error{}".format(shortname, str(e))+ "𝙳ᴀɪsʏX ʜᴀs sᴜᴄᴄᴇssғᴜʟʟʏ ʟᴏᴀᴅᴇᴅ" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
              else:
...
#kyu sir kang krne m muskil ho rhi h kya bologe toh help krdu 😂😂
                        await event.edit( "`Yᴏᴜʀ 𝙳ᴀɪsʏX ʜᴀs sᴜᴄᴄᴇssғᴜʟʟʏ ᴜɴʟᴏᴀᴅᴇᴅ` >>>" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
                    except Exception as e:
                        await event.edit("Error{}".format(shortname, str(e)) +"𝙳𝙰𝙸𝚂𝚈X ʜᴀs sᴜᴄᴄᴇssғᴜʟʟʏ ᴜɴʟᴏᴀᴅᴇᴅ"+ str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
              else:
                    shortname = event.data_match.group(1).decode("UTF-8")
    ...
    async def on_plug_in_callback_query_handler(event):
            ...
            if event.query.user_id == bot.uid or event.query.user_id == id:
                try:
                    ...
                    await event.edit("`>>> Hᴇʀᴇ ɪs ᴛʜᴇ Mᴀɪɴ Mᴇɴᴜ ᴏғ \n\n© 𝙳ᴀɪsʏ χ υѕєявσт`", buttons=buttons)
                except:
                    buttons = paginate_help(0, CMD_HELP, "helpme")
                    await event.edit("`>>> Hᴇʀᴇ ɪs ᴛʜᴇ Mᴀɪɴ Mᴇɴᴜ ᴏғ \n\n© 𝙳𝙰𝙸𝚂𝚈 χ υѕєявσт`", buttons=buttons)
            else:
                reply_pop_up_alert = "Pʟᴇᴀsᴇ Gᴇᴛ Yᴏᴜʀ Oᴡɴ Usᴇʀʙᴏᴛ (☞ ͡° ͜ʖ ͡°)☞ Fᴏʀ Mᴏʀᴇ ɪɴғᴏ Vɪsɪᴛ @DaisySupport_Official !!"
                await event.answer(reply_pop_up_alert, alert=True)
